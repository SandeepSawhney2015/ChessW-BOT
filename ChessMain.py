    ...
    if maximizing_player:
        ...
        for start_pos, piece in list(board.items()):
            if piece[0] == 'b':
                ...
                possible_moves.sort(key=lambda move: evaluate_move(board, piece, start_pos, move), reverse=True)
        ...
        for start_pos, piece in list(board.items()):
            if piece[0] == 'w':
                possible_moves = get_possible_moves(piece, start_pos)
                possible_moves.sort(key=lambda move: evaluate_move(board, piece, start_pos, move))
                for move in possible_moves:
                    end_pos = chr(move[0] + ord('a')) + str(BOARD_SIZE - move[1])
                    captured_piece = board.get(end_pos)
                    board[end_pos] = piece
...
def evaluate_move(board, piece, start_pos, move):
    end_pos = chr(move[0] + ord('a')) + str(BOARD_SIZE - move[1])
    if end_pos in board:
